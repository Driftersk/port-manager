...
if($GLOBALS['logged']) {
    ...
    try {
        ...
        $util->exec('/ip firewall nat set [find comment="PORTS:MAIN:' . $portComment . '"] disabled=yes');
        $util->exec('/ip firewall nat set [find comment="PORTS:HAIRPIN:' . $portComment . '"] disabled=yes');
    } catch(Exception $e) {
        echo json_encode(['success' => false, 'error' => 'Unable to disable port.']);
        die();
    }
