...
if($GLOBALS['logged']) {
    ...
    try {
        ...
        $util->exec('/ip firewall nat set [find comment="PORTS:MAIN:' . $portComment . '"] disabled=no');
        $util->exec('/ip firewall nat set [find comment="PORTS:HAIRPIN:' . $portComment . '"] disabled=no');
    } catch(Exception $e) {
        echo json_encode(['success' => false, 'error' => 'Unable to enable port.']);
        die();
    }
