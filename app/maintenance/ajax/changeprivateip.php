...
if($GLOBALS['logged']) {
    ...
    try {
        ...
        $util->exec('/ip firewall nat set [find comment="PORTS:MAIN:' . $portComment . '"] to-addresses=' . $privateIp);
        $util->exec('/ip firewall nat set [find comment="PORTS:HAIRPIN:' . $portComment . '"] dst-address=' . $privateIp);
    } catch(Exception $e) {
        echo json_encode(['success' => false, 'error' => 'Unable to change private ip.']);
        die();
    }
